...
function InvasionMode:InvasionOldMap()
	...
	ListenToGameEvent('game_rules_state_change', Dynamic_Wrap(InvasionMode, 'InvasionOldMapGameRulesStateChange'), self)
	ListenToGameEvent('entity_killed', Dynamic_Wrap(InvasionMode, 'OldOnEntityKilled'), self)	
 ...
 function InvasionMode:InvasionOldMapGameRulesStateChange(keys)
  local newState = GameRules:State_Get()
  if newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
    InvasionMode:InvasionOldGameStart()
  end
end


function InvasionMode:InvasionOldGameStart()
...
function InvasionMode:OldOnEntityKilled (event)
   local killedEntity = EntIndexToHScript(event.entindex_killed)
   if killedEntity:GetUnitName() == "cow" then
    if RandomInt(1, 10) > 8  then
     InvasionMode:OldCreateDrop("item_flask", killedEntity:GetAbsOrigin())
	 end
   end
   if killedEntity:GetUnitName() == "sheep" then
	 if RandomInt(1, 10) > 8  then
     InvasionMode:OldCreateDrop("item_clarity", killedEntity:GetAbsOrigin())
	 end
   end
 end
 
function InvasionMode:OldCreateDrop (itemName, pos)
